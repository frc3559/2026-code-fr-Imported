...
*/
    ...
    //Controller Inputs
    m_operatorController.rightTrigger().whileTrue(runEnd(() -> m_robotShoot.shooterSet(.27), () -> m_robotShoot.stopShooter())); //old shooting code
...
private void stopUnjamBall() { //This will run when the shooter motors get up to speed
  ...
  public Command getAutonomousCommand() {
    ...
    Command shootcmd = new ParallelCommandGroup(
      runEnd(() -> m_robotShoot.shooterSet(.32), () -> m_robotShoot.stopShooter()),
      runEnd(() -> shootBall(), () -> dontFeed()),
      waitSeconds(3).andThen(runEnd(() -> m_robotIntakeSnake.intakeSnake(1, 0, m_robotFeeder.isRunning()), () -> m_robotIntakeSnake.intakeSnake(0, 0, false)))
    ).withTimeout(8);
